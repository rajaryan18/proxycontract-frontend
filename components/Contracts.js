...
export default function Contracts() {
    ...
    return (
        <div className="my-10 mx-5 overflow-hidden w-screen p-2 md:w-3/4">
            <h1 className="mx-2 text-teal-600 text-3xl font-bold md:text-4xl lg:text-5xl">Contracts</h1>
            <div className='md:flex md:flex-row'>
                <ul className='my-8 bg-gray-300 rounded-lg'>
                    {sample.map((contract, key) => {
                        return (
                            <div>
                                <li onClick={setCurrentContract(contract.id)} className={`text-lg pl-7 pr-2 py-5 font-bold underline hover:cursor-pointer hover:bg-gray-400 md:w-1/3 md:border-black ${currContract.id === contract.id ? 'md:border-r-0 bg-gray-500' : 'md:border-r-2'}`}>{contract.name}</li>
